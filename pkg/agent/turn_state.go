...
type turnState struct {
	...
	// Active streamer for streaming LLM responses (set when stream_response is enabled)
	activeStreamer bus.Streamer
}
...
// setActiveStreamer sets the active streamer for this turn.
func (ts *turnState) setActiveStreamer(s bus.Streamer) {
	ts.mu.Lock()
	defer ts.mu.Unlock()
	ts.activeStreamer = s
}

// getActiveStreamer returns the active streamer for this turn.
func (ts *turnState) getActiveStreamer() bus.Streamer {
	ts.mu.RLock()
	defer ts.mu.RUnlock()
	return ts.activeStreamer
}

// clearActiveStreamer clears the active streamer reference.
func (ts *turnState) clearActiveStreamer() {
	ts.mu.Lock()
	defer ts.mu.Unlock()
	ts.activeStreamer = nil
}
