...
func (p *Pipeline) CallLLM(
	...
	callLLM := func(messagesForCall []providers.Message, toolDefsForCall []providers.ToolDefinition) (*providers.LLMResponse, error) {
		...
		if al.cfg.Agents.Defaults.StreamResponse && len(exec.activeCandidates) <= 1 {
			if sp, ok := exec.activeProvider.(providers.StreamingProvider); ok {
				if streamer, ok := al.bus.GetStreamer(providerCtx, ts.channel, ts.chatID); ok && streamer != nil {
					ts.setActiveStreamer(streamer)
					return al.callLLMStream(providerCtx, sp, streamer, messagesForCall, toolDefsForCall, exec.llmModel, exec.llmOpts, ts.channel, ts.chatID)
				}
			}
		}
	...
	if len(exec.response.ToolCalls) == 0 || exec.gracefulTerminal {
		...
		// Finalize active streamer on direct response
		if streamer := ts.getActiveStreamer(); streamer != nil {
			if err := streamer.Finalize(turnCtx, exec.response.Content); err != nil {
				logger.DebugCF("agent", "streamer finalize failed", map[string]any{
					"error": err.Error(),
				})
			}
			ts.clearActiveStreamer()
		}
		return ControlBreak, nil
	}
	...
	// travels the same short path as text/reasoning deltas.
	if len(exec.normalizedToolCalls) > 0 {
		if streamer := ts.getActiveStreamer(); streamer != nil {
			for _, tc := range exec.normalizedToolCalls {
				argsJSON, _ := json.Marshal(tc.Arguments)
				if err := streamer.UpdateToolCall(turnCtx, tc.ID, tc.Name, string(argsJSON)); err != nil {
